...
int main(int argc, char* argv[])
{
    ...
    static bool sVerbose     = false;true;
...
void handleRDBitem( const double & simTime, const unsigned int & simFrame, RDB_DRIVER_CTRL_t & item )
{
  fprintf( stderr, "handleRDBitem: handling driver control data\n" );
  fprintf( stderr, "    simTime = %.3lf, simFrame = %ld\n", simTime, simFrame );

  fprintf( stderr, "    id = %d\n", item.playerId);
  printf("     steeringWheel = %f rad,  accelTgt = %f m/s2\n\n", item.steeringTgt, item.accelTgt);
}
